...
"""To have functions execute automatically one would need to use OpenAIFunctionsAgent and AgentExecutor"""
...
def add_pizza(pizza_name: str, price: float) -> dict:
    ...
    Returns:
        dict: A dictionary containing the pizza's name and price.
    """

    data.append({"name": pizza_name, "price": price})
    return {"message": f"Pizza {pizza_name} with price {price} added successfully."}
...
chain = create_openai_fn_chain(
    functions=[get_pizza_info, add_pizza],
    llm=llm_chat_openai,
